<template>
  <div :style="{ textAlign: 'center' }">
    <vue-picture-swipe :items="items" :options="{ showHideOpacity:true, shareButtons: [ {id:'download', label:'Download image', url:'{{raw_image_url}}', download:true} ], rotationOn: true }"></vue-picture-swipe>
  </div>
</template>  
...
export default {
  ...
  methods: {
    buildlist () {
      var images = ['http://localhost:8081/photos/WIN_20191126_13_56_16_Pro.jpg', 'http://via.placeholder.com/640x480', 'http://via.placeholder.com/900x900']
      ...
      api.getPhotoList().then((res) => {
        ...
        images.forEach(image =>{
          ...
          img.onload = function() {
            ...
            // })
            var thumbnail = this.thumbnailify_html5(img, (thumbWidth/img.height)*img.width, thumbWidth)
            this.items.push({
              src: image,
              thumbnail: thumbnail,
              w: img.width,
